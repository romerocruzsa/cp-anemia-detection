...
def train_model(task: Task):
    def run():
        print(f"[{NODE_ID}] Training task started...")
        try:
            run_training(task.data.get("config", {}))
        except Exception as e:
            print(f"[{NODE_ID}] Error in training thread: {e}")
    
    threading.Thread(target=run).start()
    return {"id": NODE_ID, "status": "training", "message": "Training started in background."}

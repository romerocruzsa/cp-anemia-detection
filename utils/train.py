...
from utils.helper import cuda_check, sw_loss, quantile_loss
...
def train(dataloader, model, class_loss, reg1_loss, reg2_loss, optimizer, device, distiller=None, quantization="base"):
    """Trains the model using dual-encoder input and logs metrics."""
    model.to(device)
    model.train()
    total_loss = total_ce_loss = total_mse_loss = total_mae_loss = correct = total_samples = 0
    ...
    for _, (_, nail_tensor, multiclass, hb_level) in enumerate(dataloader):
        ...
        all_hb_preds.extend(reg_pred_avg.squeeze().cpu().detach().numpy())
